...
public class OpponentDaoImpl implements OpponentDao {
	...
	public void deleteOpponent(Long id1, Long id2) {
		...
		List<Long> list = entityManager.createNativeQuery("Select player_ID FROM opponent WHERE player_ID=:scrip and Opponent_ID = :prdate")
                .setParameter("scrip", id1)
                .setParameter("prdate", id2).getResultList();
		if(list.isEmpty()) {
			throw new OpponentsDoNotExist("Opponents not found"); 
			
		}
		else {
			Query query = entityManager.createNativeQuery("delete  from opponent where player_id =:id1 and opponent_id=:id2")
					.setParameter("id1", id1)
					.setParameter("id2", id2);;
			query.executeUpdate();
			
			Query query2 = entityManager.createNativeQuery("delete  from opponent where player_id =:id2 and opponent_id=:id1")
					.setParameter("id1", id1)
					.setParameter("id2", id2);;
